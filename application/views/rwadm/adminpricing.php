...
<div class="card">
  <div class="card-body">
    <div class="button-list mb-2"> 
        <a href="<?php echo base_url(adminpath.'/pricingadmin/add'); ?>" class="btn btn-primary  waves-effect waves-light text-white"> Add </a> 
        ...
        <a onclick=" $('#imageform').attr('action', '<?php echo $activeaction; ?>'); $('#imageform').submit();  " class="btn btn-warning  waves-effect waves-light text-white"> Active</a> 
        <a onclick=" $('#imageform').attr('action', '<?php echo $inactiveaction; ?>'); $('#imageform').submit();  " class="btn btn-info  waves-effect waves-light text-white">Inactive</a> 
    </div>
    <form id="imageform" name="imageform" method="post">
      <table id="state-saving-datatable" class="table activate-select dt-responsive nowrap">
        ...
        <tbody>
          <?php if($alladminpricing) {
          $i=0;
          foreach ($alladminpricing as $value) {
            $i++;
          ?>
          <tr>
            ...
            <td width="20%"><?php echo $value['pricing']; ?></td>
