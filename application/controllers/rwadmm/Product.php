...
class Product extends CI_Controller {
	...
	public function index()
	{
       ...
       $data=array();
		...
		$data['breadcrumbs'][] = array(
		'text' => 'Portfolio Photo Manager',
		'href' => base_url().adminpath.'/product'
		);
		$data['heading']="Portfolio Photo Manager";
		$this->load->model(adminpath.'/model_product');
		$data['allproducts']= array();	
    	$results = $this->model_product->getproducts();
		if ($results) {  
			foreach($results as $val){
				...
				$data['allproducts'][] = array(
					...
					'image' => $image,
					'status' => $val->status,
					'href' => base_url().adminpath.'/product/edit?id=' . $val->id
				);
			}
		}
		
		$data['deleteaction'] = base_url().adminpath.'/product/delete';
		$data['activeaction'] = base_url().adminpath.'/product/active';
		$data['inactiveaction'] = base_url().adminpath.'/product/inactive';
		$this->load->view(adminpath.'/header');
		$this->load->view(adminpath.'/product',$data);
    ...
    public function active(){
		...
		if ($this->input->server('REQUEST_METHOD') == 'POST') {
			$this->load->model(adminpath.'/model_product');
			$this->model_product->active($_POST['table_records']);
    	}
    	redirect(adminpath.'/product');
    }
    public function inactive(){

		if ($this->input->server('REQUEST_METHOD') == 'POST') {
			$this->load->model(adminpath.'/model_product');
			$this->model_product->inactive($_POST['table_records']);
    	}
    	redirect(adminpath.'/product');
    }
    public function delete(){
		...
		if ($this->input->server('REQUEST_METHOD') == 'POST') {
			$this->load->model(adminpath.'/model_product');
			$this->model_product->delete($_POST['table_records']);
    	}
    	redirect(adminpath.'/product');
    }
    public function add() {
		 ...
		 if ($this->input->server('REQUEST_METHOD') == 'POST') {
			...
			$postdata['author'] = $this->input->post('author');
			$postdata['sku'] = $this->input->post('sku');
			$postdata['categorydata'] = $this->input->post('categorydata');
			...
			$postdata['deturl'] = $this->input->post('deturl');
			$postdata['price'] = $this->input->post('price');
			$postdata['bookformat'] = $this->input->post('bookformat');			
			$postdata['feature'] = $this->input->post('feature');
			$postdata['ordernum'] = $this->input->post('ordernum');			
			$postdata['date_added'] = date('Y-m-d H:i:s');
			$postdata['status'] = '1';
			$this->model_product->add($postdata,$category);
			$this->session->set_flashdata('productnotify', 'Username and Password not Valid.');
			redirect(adminpath.'/product');
		}
		$this->getform();
     ...
     public function edit() {
		 ...
		 if ($this->input->server('REQUEST_METHOD') == 'POST') {
			...
			$postdata['status'] = '1';
			$this->model_product->edit($this->input->get('id'),$postdata,$category);
			$this->session->set_flashdata('productnotify', 'Username and Password not Valid.');
			redirect(adminpath.'/product');
		}
		$this->getform();
    ...
    public function getform() {
		...
		$data['breadcrumbs'][] = array(
		'text' => 'Add Portfolio Photo',
		'href' => '#'
		);
		$data['heading']="Add Portfolio Photo";
		$data['action'] = base_url().adminpath.'/product/add';
		} else {
		$data['breadcrumbs'][] = array(
		'text' => 'Edit Portfolio Photo',
		'href' => '#'
		);
		$data['heading']="Edit Portfolio Photo";
		$data['action'] = base_url().adminpath.'/product/edit?id=' . $this->input->get('id');
		}
		
		if ($this->input->get('id') && $this->input->server('REQUEST_METHOD') != 'POST') {
      		$info = $this->model_product->getproduct($this->input->get('id'));
    	}
		...
		if ($this->input->post('categorydata')) {
      		$data['categorydata'] = $this->input->post('categorydata');
    	} elseif (isset($info)) {
		...
		if ($this->input->post('bookformat')) {
      		$data['bookformat'] = $this->input->post('bookformat');
    	} elseif (isset($info)) {
			$data['bookformat'] = $info->bookformat;
		} else {
      		$data['bookformat'] = '';
    	}
		...
		if ($this->input->post('deturl')) {
      		$data['deturl'] = $this->input->post('deturl');
    	} elseif (isset($info)) {
		...
		if ($results) {  
			foreach($results as $val){
					...
					foreach($subresults as $val1){
						...
						if ($subresults1) {
							foreach($subresults1 as $val2){
								$catchecked="";
								if($this->model_product->getproductcategory($data['id'],$val2->id)){
									$catchecked="checked";
								} 
								$subcategory1[] = array(
									'id' => $val2->id,
						...
						$catchecked="";
							if($this->model_product->getproductcategory($data['id'],$val1->id)){
								$catchecked="checked";
							} 
				...
				$catchecked="";
				if($this->model_product->getproductcategory($data['id'],$val->id)){
					$catchecked="checked";
				}
				$data['allpcategory'][] = array(
					'id' => $val->id,
		...
		$data['cancel'] = base_url().adminpath.'/product';
		
		$this->load->view(adminpath.'/header');
		$this->load->view(adminpath.'/productform',$data);
		$this->load->view(adminpath.'/footer');
