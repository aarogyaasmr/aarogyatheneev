...
class Testimonials extends CI_Controller {
    ...
    public function add() {
		 if ($this->input->server('REQUEST_METHOD') == 'POST') {
			...
			if($_FILES['image']['name'] <>'')
		 	{
				$allowed_filetypes = array('.pdf','.doc','.svg','.gif','.jpg','.jpeg','.png');			
			...
			$postdata['ttype'] = $this->input->post('ttype');
			...
			$this->model_testimonials->add($postdata);
    ...
    public function edit() {
		 if ($this->input->server('REQUEST_METHOD') == 'POST') {
			...
			$postdata['status'] = '1';
			$this->model_testimonials->edit($this->input->get('id'),$postdata);
			$this->session->set_flashdata('imagenotify', 'Username and Password not Valid.');
			redirect(adminpath.'/testimonials');
		}
		$this->getform();
    ...
    public function getform() {
    	...
    	if ($this->input->post('ttype')) {
      		$data['ttype'] = $this->input->post('ttype');
    	} elseif (isset($info)) {
			$data['ttype'] = $info->ttype;
		} else {
      		$data['ttype'] = '';
    	}
