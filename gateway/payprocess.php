<?php
    ...
    if(isset($_POST['submit'])) {
        ...
        $date = date("Y-m-d");
        // echo
        $sql="insert into payment_enquiry set name='".$name."',email='".$email."',phone='".$phone."',age='".$age."',weight='".$weight."',pupose='".$pupose."',pay_status='Pending',price='".$payment."',orderid='".$orderid."',payment_type='Direct Payment',plan='".$plan."',coupon='".$coupon."',source='".$source."',date='".$date."'";
        // exit;
        if(mysqli_query($connnect,$sql)) {
            ...
            if ($row[0] == 0) {
                $sqlcustomer="insert into customers set name='".$name."',email='".$email."',phone='".$phone."',orderid='".$orderid."',coupon='".$coupon."',source='".$source."',plan='".$plan."',date='".$date."'";
                $result = mysqli_query($connnect,$sqlcustomer);
                $lastInsertId = mysqli_insert_id($connnect);
